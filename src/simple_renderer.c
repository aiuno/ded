...
static_assert(COUNT_SIMPLE_SHADERS == 4,
              "The amount of fragment shaders has changed");
...
static bool compile_shader_file(const char *file_path, GLenum shader_type,
                                GLuint *shader) {
  bool result = true;
...
static_assert(COUNT_UNIFORM_SLOTS == 4,
              "The amount of the shader uniforms have change. Please update "
              "the definition table accordingly");
...
static void get_uniform_location(GLuint program,
...
void simple_renderer_init(Simple_Renderer *sr) {
  ...
  if (!compile_shader_file(vert_shader_file_path, GL_VERTEX_SHADER,
                           &shaders[0])) {
    exit(1);
  }

  for (int i = 0; i < COUNT_SIMPLE_SHADERS; ++i) {
    if (!compile_shader_file(frag_shader_file_paths[i], GL_FRAGMENT_SHADER,
                             &shaders[1])) {
      exit(1);
    }
...
// TODO: Don't render triples of vertices that form a triangle that is
...
//
// This might not be what we want at the end of the day, though. Because in case
// of a lot of triangles we end up iterating each of them at least once and
// doing camera trasformations on the CPU (which is something we do on GPU
// already).
//
// It would be probably better if such culling occurred on a higher level of
// abstractions. For example in the Editor. For instance, if the Editor noticed
// that the line it is currently rendering is below the screen, it should stop
// rendering the rest of the text, thus never calling simple_renderer_vertex()
// for a potentially large amount of vertices in the first place.
void simple_renderer_vertex(Simple_Renderer *sr, Vec2f p, Vec4f c, Vec2f uv) {
    ...
    if (sr->vertices_count >= SIMPLE_VERTICES_CAP) simple_renderer_flush(sr);
#else
  // NOTE: it is better to just crash the app in this case until the culling
  // described above is sorted out.
...
void simple_renderer_set_shader(Simple_Renderer *sr, Simple_Shader shader) {
  ...
  glUniform2f(sr->uniforms[UNIFORM_SLOT_RESOLUTION], sr->resolution.x,
              sr->resolution.y);
