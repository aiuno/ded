...
static_assert(SIMPLE_VERTICES_CAP % 3 == 0,
              "Simple renderer vertex capacity must be divisible by 3. We are "
              "rendring triangles after all.");

typedef enum {
  SHADER_FOR_COLOR = 0,
  SHADER_FOR_IMAGE,
  SHADER_FOR_TEXT,
  SHADER_FOR_EPICNESS, // This is the one that does that cool rainbowish
                       // animation
